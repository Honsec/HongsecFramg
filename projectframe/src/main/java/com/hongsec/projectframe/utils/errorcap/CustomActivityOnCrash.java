...
import com.hongsec.projectframe.BaseApplication;
...
public final class CustomActivityOnCrash {
    ...
    private static final String CAOC_HANDLER_PACKAGE_NAME = "com.miniram.donpush.cn";//TODO 고쳐야 됨 내 패키지명 으로
        ...
        try {
            if (context == null) {
                ...
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                    LogUtils.e(TAG, "CustomActivityOnCrash will be installed, but may not be reliable in API lower than 14");
                }
                ...
                if (oldHandler != null && oldHandler.getClass().getName().startsWith(CAOC_HANDLER_PACKAGE_NAME)) {
                    LogUtils.e(TAG, "You have already installed CustomActivityOnCrash, doing nothing!");
                } else {
//                    if (oldHandler != null && !oldHandler.getClass().getName().startsWith(DEFAULT_HANDLER_PACKAGE_NAME)) {
//                        LogUtils.e(TAG, "IMPORTANT WARNING! You already have an UncaughtExceptionHandler, are you sure this is correct? If you use ACRA, Crashlytics or similar libraries, you must initialize them AFTER CustomActivityOnCrash! Installing anyway, but your original handler will not be called.");
//                    }
                    ...
                    Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
                        ...
                        public void uncaughtException(Thread thread, final Throwable throwable) {
                            ...
                            if (isStackTraceLikelyConflictive(throwable, errorActivityClass)) {
                                LogUtils.e(TAG, "Your application class or your error activity have crashed, the custom activity will not be launched!");
                            } else {
                                if (launchErrorActivityWhenInBackground || !isInBackground) {
                                    ...
                                    DB_APP db_app = new DB_APP(application.getApplicationContext());
                                    db_app.openDB();

                                    db_app.InsertError(BaseApplication.getStackTrace(application,throwable));
                    ...
                    LogUtils.d(TAG, "CustomActivityOnCrash has been installed.");
                }
            }
        } catch (Throwable t) {
        }
...
//        return intent.getIntExtra(CustomActivityOnCrash.EXTRA_IMAGE_DRAWABLE_ID, R.drawable.pig_01_01);
//    }
...
//    }
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.miniram.donpush.cn.RESTART" />,
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.miniram.donpush.cn.RESTART" />,
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.miniram.donpush.cn.ERROR" />,
    ...
    private static Class<? extends Activity> guessErrorActivityClass(Context context) {
        ...
        //Else, get the default launcher activity
        if (resolvedActivityClass == null) {
            resolvedActivityClass = CustomCrashAct.class;
        }
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.miniram.donpush.cn.ERROR" />,
     * If there is no activity with that intent filter, this returns null.
     *
     * @param context A valid context. Must not be null.
     * @return A valid activity class, or null if no suitable one is found
